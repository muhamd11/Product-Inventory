...
{
    public class ProductStoreDeleteResponse : BaseResponse
    {
        [JsonPropertyOrder(4)]
        public ProductInfoDetails productInfoData { get; set; }

        public ProductStoreDeleteResponse CreateResponse(string message, EnumStatus status, ProductInfoDetails? data = null)
        {
            if (data == null)
                return new () { Message = message, Status = status };

            return new () { Message = message, Status = status, productInfoData = data };
        }
    }
}
